...
async def channel_receive_handler(bot: Client, broadcast: Message):
    try:
        ...
        msg = await broadcast.copy(chat_id=BIN_CHANNEL)
        raw_stream = f"{URL}watch/{msg.id}/sujanbotz.mkv?hash={get_hash(msg)}"
        ...
        buttons_list = [
            ...
            [InlineKeyboardButton('• Cʜᴇᴄᴋ Hᴇʀᴇ Tᴏ Gᴇᴛ Fɪʟᴇ •', url=file_link)]
        ]
        if IS_SHORTLINK:
            buttons_list.append([
                InlineKeyboardButton("• Hᴏᴡ Tᴏ Oᴘᴇɴ •", url=HOW_TO_OPEN)
            ])
