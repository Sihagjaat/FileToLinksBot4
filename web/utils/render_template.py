...
import logging
import aiohttp
from web.utils.Template import sujanbotz_template
from info import *
...
async def render_page(id: str, secure_hash: str, src: str = None) -> str:
    ...
    try:
        file = await Websujanbot.get_messages(int(BIN_CHANNEL), int(id))
        file_data = await get_file_ids(Websujanbot, int(BIN_CHANNEL), int(id))
    ...
    if tag in ["video", "audio"]:
        template_file = os.path.join("web", "template", "websujan.html")
    else:
    ...
    file_name = file_data.file_name.replace("_", " ") if file_data.file_name else f"sujan_File_{id}.mkv"
    ...
    return template.render(
        ...
        template_ne=sujanbotz_template.NAME,
        disclaimer=sujanbotz_template.DISCLAIMER,
        report_link=sujanbotz_template.REPORT_LINK,
        colours=sujanbotz_template.COLOURS,
                                    )
